...
public class MpdPlayer implements MusicPlayer {
    ...
    @Override
    public TaskHandle updatePlayData(List<String> uris, ResponseReceiver<Boolean> responseReceiver) {
        MpdUpdatePlayDataCommand command = new MpdUpdatePlayDataCommand(uris);
        return command.execute(databaseHelper, connection, new MpdDatabaseCommandReceiver<Boolean>() {
            @Override
            public void build() {
                responseReceiver.buildingDatabase();
